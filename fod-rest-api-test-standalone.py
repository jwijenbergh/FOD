...
def create_fodcon_handle(proto, server_addr, use_ssl_verify, token, username):
  ...
  print "server_addr="+str(server_addr)
  print "use_ssl_verify="+str(use_ssl_verify)
...
def fod_rule_api__get_routes1(fod_con, id=None):
  ...
  if id==None:
    ...
    print json_hashed.get("test_port0_T03QQL")
    print json.dumps(json_hashed.get("test_port0_T03QQL"), indent=4, sort_keys=True)
...
def fod_set_up_rule(fod_con, rule, routes):
  ...
  try:
    ...
    #exit(1)
    print "rule_obj items:", type(rule_obj)
    #print "rule_obj items:", len(rule_obj)
    #assert(len(rule_obj) == 1)
    #rule_obj = rule_obj[0]
  except Exception, e:
    print "exception in fod_rule_api__create_rule: "+str(e)
    raise
  ...
  for route in routes:
    try:
      ...
      if status_code==0 or status_code==201:
        ...
        print "for source="+str(route['source'])+", destination="+str(route['destination'])+" :"
        print "route creation failed: status_code="+str(status_code)+" : "+str(route_obj)
    except Exception, e:
      print "exception in fod_rule_api__create_route: attacker_ipv4="+str(route['source'])+" :"+str(e)
      raise  

  if count_created_routes==0:
    print "no routes created for rule, aborting rule creation"
    ...
    try:
      ...
      if status_code!=0 and status_code!=204:
        print "rule deletion failed: status_code="+str(status_code)+" : "+str(response1)

    except Exception, e:
      print "exception in fod_rule_api__change_rule__partial: "+str(e)
      raise
